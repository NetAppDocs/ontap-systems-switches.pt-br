...
summary: 'Após concluir a configuração inicial, instale o arquivo de configuração de referência (RCF) e o arquivo de licença no switch de cluster Broadcom BES-53248.' 
---
= Instale o arquivo RCF e o arquivo de licença no switch de cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale o arquivo de configuração de referência (RCF) e o arquivo de licença juntos no switch de cluster Broadcom BES-53248. A partir do EFOS 3.12.0.1, você pode instalar ambos os arquivos após concluir a configuração inicial do switch.
...
* O RCF atual, disponível na link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Comutadores de cluster Broadcom"^] página.
...
. Copie o arquivo RCF para a memória flash de inicialização do switch cs2 usando um dos seguintes protocolos de transferência: FTP, HTTP, TFTP, SFTP ou SCP.
+
Este exemplo mostra o uso do HTTP para copiar um arquivo RCF para a memória flash de inicialização no switch cs2:
...
----
====
. No interruptor, verifique se as alterações foram aplicadas:
+
[source, cli]
----
show running-config
...
--
====
. [[passo21]]No switch de cluster cs1, desligue as portas conectadas às portas de cluster dos nós.
+
...
.O que vem a seguir?
Após instalar o RCF e o arquivo de licença, você pode link:configure-ssh.html["habilitar SSH"].
