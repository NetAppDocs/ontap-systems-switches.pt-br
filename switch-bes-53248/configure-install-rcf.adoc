...
summary: 'Instale o arquivo de configuração de referência (RCF) no switch de cluster Broadcom BES-53248 após concluir a configuração inicial e aplicar quaisquer licenças necessárias.' 
---
= Instale o RCF no switch de cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale o arquivo de configuração de referência (RCF) no switch de cluster Broadcom BES-53248 após concluir a configuração inicial e aplicar as licenças necessárias. O RCF define as atribuições de porta, VLANs e configurações de rede para a operação do cluster.


CAUTION: Para o EFOS 3.12 e versões posteriores, siga os passos de instalação em link:efos-install-rcf-license-file.html["Instale o arquivo de configuração (RCF) e o arquivo de licença"].
...
* O arquivo RCF atual, disponível na link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Comutadores de cluster Broadcom"^] página.
...
* É necessária uma conexão de console com o switch ao instalar o RCF a partir de suas configurações de fábrica. Este requisito é opcional caso você tenha utilizado o artigo da Knowledge Base link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Como limpar a configuração de um switch de interconexão Broadcom mantendo a conectividade remota"^] para limpar a configuração, previamente.


.Documentação sugerida
Consulte a tabela de compatibilidade de switches para as versões suportadas do ONTAP e do RCF. Consulte link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["Download do software EFOS"^]. Observe que pode haver dependências de comandos entre a sintaxe de comandos no RCF e a encontrada nas versões do EFOS.
...
* Os exemplos neste procedimento usam quatro nós. Esses nós usam duas portas de interconexão de cluster 10GbE `e0a` e `e0b`. Consulte a link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] para verificar as portas de cluster corretas em suas plataformas.
...
CAUTION: Antes de instalar uma nova versão de software do switch e RCFs, use o artigo da Knowledge Base link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Como limpar a configuração de um switch de interconexão Broadcom mantendo a conectividade remota"^]. Se for necessário apagar completamente as configurações do switch, será preciso realizar a configuração básica novamente. Você deve estar conectado ao switch usando o console serial, pois uma exclusão completa da configuração redefine a configuração da rede de gerenciamento.
...
. Observe o seguinte:
+
.. Se licenças de porta adicionais tiverem sido instaladas no switch, você deve modificar o RCF para configurar as portas licenciadas adicionais. Consulte link:configure-licenses.html#activate-newly-licensed-ports["Ativar portas recém-licenciadas"] para obter detalhes.
