...
summary: 'Pode atualizar o ficheiro de configuração de referência (RCF) após configurar a central de cluster BES-53248 e depois de aplicar quaisquer novas licenças.' 
---
= Atualizar o ficheiro de configuração de referência (RCF)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode atualizar o RCF (Reference Configuration File) após atualizar o EFOS do switch de cluster BES-53248 e depois de aplicar novas licenças.
...
* Consulte os guias de software e atualização apropriados disponíveis no https://www.broadcom.com/support/bes-switch["Broadcom"^] site para obter a documentação completa sobre os procedimentos de atualização e downgrade do switch BES-53248.
...
CAUTION: Antes de instalar uma nova versão de software de switch e RCFs, use o artigo da base de dados de Conhecimento https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Como limpar a configuração em um switch de interconexão Broadcom, mantendo a conetividade remota"^] . Se você precisar apagar completamente as configurações do switch, então você precisará executar a configuração básica novamente. Você deve estar conetado ao switch usando o console serial, já que uma eliminação completa da configuração redefine a configuração da rede de gerenciamento.
...
show isdp neighbor
----
. No switch CS2, encerre as portas conetadas às portas do cluster dos nós. Por exemplo, se as portas 0/1 a 0/16 estiverem conetadas a nós ONTAP:
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. Copie o RCF para o flash de inicialização do switch CS2 usando um dos seguintes protocolos de transferência: FTP, TFTP, SFTP ou SCP.
+
...
----
====
. Examine a saída do banner a partir do `show clibanner` comando. Você deve ler e seguir estas instruções para garantir a configuração e o funcionamento corretos do switch.
+
`show clibanner`
...
----
. No switch de cluster CS2, abra as portas conetadas às portas de cluster dos nós.
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[step21]]repita os passos 1 a 20 no interrutor CS1.
