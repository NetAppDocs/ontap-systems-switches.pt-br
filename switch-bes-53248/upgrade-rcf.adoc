...
summary: Você pode atualizar o RCF após configurar o switch de cluster BES-53248 e após aplicar quaisquer novas chaves da licença. 
---
= Atualizar a versão RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode atualizar o RCF após atualizar o BES-53248 cluster switch EFOS e após aplicar quaisquer novas licenças.
...
* Consulte os guias de software e upgrade disponíveis no link:https://www.broadcom.com/support/bes-switch["Broadcom"^] site para obter a documentação completa sobre os procedimentos de upgrade e downgrade do switch BES-53248.
...
show isdp neighbor
----
. No switch cs2, desligue as portas conectadas às portas do cluster dos nós.  Por exemplo, se as portas 0/1 a 0/16 estiverem conectadas a nós ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. Observe o seguinte:
+
.. Se licenças de porta adicionais tiverem sido instaladas no switch, você deve modificar o RCF para configurar as portas licenciadas adicionais. Consulte link:configure-licenses.html#activate-newly-licensed-ports["Ativar portas recém-licenciadas"] para obter detalhes. No entanto, ao atualizar para o RCF 1.12 ou posterior, as modificações não são mais necessárias porque todas as interfaces agora estão pré-configuradas.
...
CAUTION: O arquivo deve ser nomeado  `reference-config.scr` ao fazer o upload e aplicar o arquivo RCF, caso contrário, a etapa de aplicação do script falhará.

. Copie o arquivo RCF para o switch cs2 usando um dos seguintes protocolos de transferência: HTTP, HTTPS, FTP, SFTP ou SCP.
+
...
[listing, subs="+quotes"]
----
(cs2)# *copy sftp://172.19.2.1/BES-53248_RCF_v1.9-Cluster-HA.txt nvram:reference-config.scr*
Remote Password:**
Mode........................................... SFTP
...
--
====
. [[passo 11]]Examine a saída do banner do `show clibanner` comando. Você deve ler e seguir estas instruções para garantir a configuração e operação corretas do switch.
+
`show clibanner`
...
----
. No switch de cluster cs2, ative as portas conectadas às portas de cluster dos nós.
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[passo21]]Repita os passos 1 a 20 no interruptor cs1.
