...
summary: Configure a coleta de logs no switch de cluster Broadcom BES-53248 para permitir que o CSHM colete logs do switch para depuração e AutoSupport reporting. 
---
= Configurar a coleta de logs para o switch de cluster Broadcom BES-53248
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Configure a coleta de logs no switch de cluster Broadcom BES-53248 para permitir que o monitor de integridade do switch Ethernet (CSHM) colete logs do switch para depuração e para relatórios do AutoSupport. Este procedimento configura tanto a coleta de logs de suporte sob demanda quanto a coleta periódica de dados a cada hora.
