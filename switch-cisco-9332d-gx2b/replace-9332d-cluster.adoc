...
summary: Substitua um switch Cisco Nexus 9332D-GX2B defeituoso em um sistema AFX sem interromper a operação. Este é um procedimento não disruptivo. 
---
= Substitua um switch Cisco Nexus 9332D-GX2B
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um switch Cisco Nexus 9332D-GX2B defeituoso em um sistema AFX sem interromper a operação. Este procedimento não disruptivo (NDU) envolve preparar um switch de substituição e restaurar a configuração do sistema.
...
** A personalização inicial do switch está concluída, conforme detalhado em link:configure-complete-initial-setup-9332d.html["Conclua a configuração inicial do switch"].
+
...
* Execute uma manutenção AutoSupport antes e depois da manutenção para desativar a criação de chamados durante o período de manutenção. Consulte este artigo da Knowledge Base https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU92["SU92: Como suprimir a criação automática de chamados durante janelas de manutenção programadas"^] para obter mais detalhes.
...
newcs2(config-if-range)# *shutdown*
newcs2(config)# *interface e1/17/1-4,e1/18/1-4,e1/19/1-4,e1/20/1-4,e1/21/1-4,e1/22/1-4,e1/23/1-4*
newcs2(config-if-range)# *shutdown*
newcs2(config)# *interface e1/24/1-4,e1/25/1-4,e1/26/1-4,e1/27/1-4,e1/28/1-4,e1/29/1-4,e1/30/1-4*
newcs2(config-if-range)# *shutdown*
...
----
====
. Ative todas as portas, de Eth1/1 a Eth1/30, no switch newcs2.
+
.Mostrar exemplo
[%collapsible]
====
...
newcs2# *config*
newcs2(config)# *interface e1/1/1-4,e1/2/1-4,e1/3/1-4,e1/4/1-4,e1/5/1-4,e1/6/1-4,e1/7/1-4,e1/8/1-4*
newcs2(config-if-range)# *no shutdown*
newcs2(config)# *interface e1/9/1-4,e1/10/1-4,e1/11/1-4,e1/12/1-4,e1/13/1-4,e1/14/1-4,e1/15/1-4,e1/16/1-4*
newcs2(config-if-range)# *no shutdown*
newcs2(config)# *interface e1/17/1-4,e1/18/1-4,e1/19/1-4,e1/20/1-4,e1/21/1-4,e1/22/1-4,e1/23/1-4*
newcs2(config-if-range)# *no shutdown*
