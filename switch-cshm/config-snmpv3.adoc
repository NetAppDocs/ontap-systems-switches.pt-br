...
summary: 'Configure o SNMPv3 em seus switches de rede para habilitar a comunicação segura com o monitor de integridade do switch Ethernet (CSHM).' 
---
= Configurar o SNMPv3 para monitoramento da integridade do switch de rede Ethernet
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Configure o SNMPv3 em seus switches de rede para habilitar a comunicação segura e autenticada com o monitor de integridade do switch Ethernet (CSHM). O SNMPv3 fornece sondagem criptografada do status do switch para monitoramento de integridade no ONTAP.

O monitor de integridade do switch Ethernet (CSHM) utiliza SNMP para monitorar a integridade e o desempenho dos switches de rede. Por padrão, o SNMPv2c é configurado automaticamente através do Reference Configuration File (RCF). O SNMPv3 é mais seguro que o SNMPv2 porque introduz recursos de segurança robustos, como autenticação, criptografia e integridade de mensagens, que protegem contra acesso não autorizado e garantem a confidencialidade e integridade dos dados durante a transmissão.
...
+
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-82891["Para monitoramento de integridade do ONTAP em switches Cisco , o tráfego SNMPv2 ainda pode ser observado após a migração para o SNMPv3."^]
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-119131["Alertas falsos positivos de ventoinha e energia do switch quando ocorrem falhas SNMP."^]
...
----
. Configure o usuário SNMPv3 no lado do ONTAP :
+
[source, cli]
----
...
----
. Após aguardar o período de consulta do CSHM, verifique se o número de série está preenchido para o switch Ethernet.
+
[source, cli]
----
...
.switches Cisco
--
Configure um nome de usuário SNMPv3 SNMPv3_USER nos switches Cisco Nexus:
...
cluster1::*>
----
. Verifique se o número de série a ser consultado com o usuário SNMPv3 recém-criado é o mesmo detalhado na etapa anterior, após a conclusão do período de pesquisa CSHM.
+
[source, cli]
----
system switch ethernet polling-interval show
...
.NVIDIA - CL 5.4.0
--
Configure um nome de usuário SNMPv3 SNMPv3_USER em switches NVIDIA Cumulus executando CL 5.4.0:
...
.NVIDIA - CL 5.11.0+
--
Configure um nome de usuário SNMPv3 SNMPv3_USER em switches NVIDIA Cumulus executando a CLI 5.11.0 ou posterior:
