...
O CSHM (Ethernet Switch Health Monitor) utiliza SNMP para monitorar a integridade e o desempenho dos switches de cluster e armazenamento. Por predefinição, o SNMPv2c é configurado automaticamente através do ficheiro de configuração de referência (RCF). O SNMPv3 é mais seguro do que o SNMPv2 porque introduz recursos de segurança robustos, como autenticação, criptografia e integridade de mensagens, que protegem contra acesso não autorizado e garantem a confidencialidade e integridade dos dados durante a transmissão.
