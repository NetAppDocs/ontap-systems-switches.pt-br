...
* Para coleta de logs com switches Broadcom e Cisco, um novo usuário deve ser criado no switch para cada configuração com coleta de logs ativada. Esses switches não suportam várias chaves SSH para o mesmo usuário. Qualquer configuração adicional de coleção de logs executada substitui quaisquer chaves SSH pré-existentes para o usuário.
