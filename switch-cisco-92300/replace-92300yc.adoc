...
summary: Substitua um switch de cluster Cisco Nexus 92300YC defeituoso em uma rede de cluster. Este é um procedimento não disruptivo. 
---
= Substitua um switch de cluster Cisco Nexus 92300YC
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um switch de cluster Cisco Nexus 92300YC defeituoso em uma rede de cluster sem interromper a operação do cluster. Este procedimento não disruptivo (NDU) envolve preparar um switch de substituição e migrar as conexões do switch com falha.
...
.Antes de começar
Antes de substituir o switch, certifique-se de que:
...
* Para o switch de substituição Nexus 92300YC:
+
...
** A personalização inicial do switch está concluída, conforme detalhado em: link:configure-install-initial.html["Configurar o switch Cisco Nexus 92300YC"].
+
...
----
====
. Remova todos os cabos do switch Nexus 92300YC cs2 e, em seguida, conecte-os às mesmas portas do switch Nexus 92300YC newcs2.
. Ative as portas ISL 1/65 e 1/66 entre os switches cs1 e newcs2 e, em seguida, verifique o status de operação do port channel.
+
Port-Channel deve indicar Po1(SU) e as portas de membro devem indicar Eth1/65(P) e Eth1/66(P).
