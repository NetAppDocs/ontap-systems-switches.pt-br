...
summary: Substitua um switch Cisco Nexus 9364D-GX2A defeituoso em um sistema AFX sem interromper a operação. Este é um procedimento não disruptivo. 
---
= Substitua um switch Cisco Nexus 9364D-GX2A
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um switch Cisco Nexus 9364D-GX2A defeituoso em um sistema AFX sem interromper a operação. Este procedimento não disruptivo (NDU) envolve preparar um switch de substituição e restaurar a configuração do sistema.
...
* No switch de substituição Nexus 9364D-GX2A:
+
...
** As conexões do nó são as portas 1/1 a 1/62.
** Todas as portas Inter-Switch Link (ISL) estão desabilitadas nas portas 1/63 e 1/64.
** O arquivo de configuração de referência (RCF) desejado e a imagem do sistema operacional NX-OS são carregados no switch.
** A personalização inicial do switch está concluída, conforme detalhado em link:configure-complete-initial-setup-9364d.html["Conclua a configuração inicial do switch"].
+
...
* Os nomes dos switches Nexus 9364D-GX2A existentes são cs1 e cs2.
* O nome do novo switch Nexus 9364D-GX2A é newcs2.
...
newcs2(config-if-range)# *shutdown*
newcs2(config)# *interface e1/41/1-4,e1/42/1-4,e1/43/1-4,e1/44/1-4,e1/45/1-4,e1/46/1-4,e1/47/1-4,e1/48/1-4*
newcs2(config-if-range)# *shutdown*
...
. Desligue as portas ISL 1/63 e 1/64 no switch Nexus 9364D-GX2A cs1.
+
[listing, subs="+quotes"]
----
cs1# *config*
...
cs1(config)# *exit*
----
. Remova todos os cabos do switch Nexus 9364D-GX2B cs2 e conecte-os às mesmas portas no switch Nexus 9364D-GX2A newcs2.
. Abra as portas ISLs 1/63 e 1/64 entre os switches cs1 e newcs2 e, em seguida, verifique o status da operação do canal da porta.
+
Port-Channel deve indicar Po1(SU) e Member Ports deve indicar Eth1/63(P) e Eth1/64(P).

+
.Mostrar exemplo
[%collapsible]
====
Este exemplo habilita as portas ISL 1/63 e 1/64 e exibe o resumo do canal de porta no switch cs1:
...
----
====
. Ative todas as portas do switch newcs2.
+
.Mostrar exemplo
[%collapsible]
====
