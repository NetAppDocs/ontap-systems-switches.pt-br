...
[role="lead"]
Migre de switches de cluster Cisco para switches de cluster NVIDIA SN2100 sem interromper a operação do cluster.
...
Você deve estar ciente de certas informações de configuração, conexões de porta e requisitos de cabeamento ao substituir alguns switches de cluster Cisco mais antigos por switches de cluster NVIDIA SN2100. Consulte link:configure-overview-sn2100-cluster.html["Visão geral da instalação e configuração dos switches NVIDIA SN2100"].
...
Para obter detalhes sobre as portas suportadas e suas configurações, consulte o link:https://hwu.netapp.com/["Hardware Universe"^] .
...
NOTE: Se você estiver alterando a velocidade das portas de cluster e0a e e1a em sistemas AFF A800 ou AFF C800, poderá observar o recebimento de pacotes malformados após a conversão de velocidade. Consulte link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339["Erro 1570339"^] e o artigo da Knowledge Base link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE["Erros de CRC em portas T6 após conversão de 40GbE para 100GbE"^] para obter orientações.
...
+
Cada LIF deve exibir `up/up` para `Status Admin/Oper` e é verdade para `Is Home` .
