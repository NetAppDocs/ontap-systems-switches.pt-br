...
summary: 'Atualize o script do arquivo de configuração de referência (RCF) nos switches de cluster NVIDIA SN2100 quando uma versão mais recente estiver disponível.' 
---
= Atualize o script RCF nos switches de cluster NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Atualize o script do arquivo de configuração de referência (RCF) nos switches de cluster NVIDIA SN2100 quando uma versão mais recente estiver disponível.

.Antes de começar
Antes de atualizar o script RCF, certifique-se de que os seguintes itens estejam disponíveis no switch:
...
NOTE: Embora `scp` Se o método utilizado for o do exemplo, você pode usar o seu método preferido de transferência de arquivos.
...
NOTE: Na etapa 3 *Atualizando o arquivo MOTD* acima, o comando `cat /etc/issue` é executado.  Isso permite verificar o nome do arquivo RCF, a versão do RCF, as portas a serem usadas e outras informações importantes no banner do RCF.
...
NOTE: Enquanto `scp` Se o método utilizado for o do exemplo, você pode usar o seu método preferido de transferência de arquivos.
