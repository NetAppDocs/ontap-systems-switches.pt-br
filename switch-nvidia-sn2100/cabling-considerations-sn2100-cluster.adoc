...
summary: Revise as considerações de cabeamento e configuração dos switches de cluster NVIDIA SN2100 antes da implementação. 
---
= Analise as considerações de cabeamento e configuração para switches de cluster NVIDIA SN2100.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Revise as considerações de cabeamento e configuração dos switches de cluster NVIDIA SN2100 antes da implementação.
...
--
====
Consulte o artigo da Knowledge Base link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Systems/Fabric_Interconnect_and_Management_Switches/NVIDIA_SN2100_switch_fails_to_connect_using_40_100GbE_copper_cable["O switch SN2100 não consegue se conectar usando cabos de cobre 40/100GbE."^] para obter mais detalhes.
...
Ao usar adaptadores QSFP+ (40GbE) para SFP+ (10GbE) ou adaptadores QSFP28 (100GbE) para SFP28 (25GbE) (QSA), insira-os em portas de switch 40GbE/100GbE não breakout (swp3-swp14). Não insira o módulo QSA em uma porta configurada para breakout.

Quando um módulo QSA é usado para conectar-se às portas de cluster 10GbE/25GbE em uma plataforma, a conexão pode não ser estabelecida.

Para resolver esse problema, faça o seguinte:

* Para 10GbE, defina manualmente a velocidade da conexão para 10000 e desative a negociação automática.
* Para conexões de 25GbE, defina manualmente a velocidade da conexão para 25000 e desative a negociação automática.
...
Dependendo do transceptor na porta do switch, pode ser necessário definir a velocidade na interface do switch para uma velocidade fixa. Se estiver usando portas breakout de 10GbE e 25GbE ou um módulo QSA, verifique se a negociação automática está desativada e configure a velocidade da interface no switch.
...
--
====
Consulte o link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] e o artigo da Knowledge Base link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["Que informações adicionais preciso para instalar meu equipamento que não está no HWU?"^] para mais informações.
