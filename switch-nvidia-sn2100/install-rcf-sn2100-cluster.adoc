...
summary: 'Instale ou atualize o script do arquivo de configuração de referência (RCF) nos switches de cluster NVIDIA SN2100.' 
---
= Instale ou atualize o script RCF nos switches de cluster NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale ou atualize o script do arquivo de configuração de referência (RCF) nos switches de cluster NVIDIA SN2100 para configurar atribuições de porta e configurações de rede.

.Antes de começar
Antes de instalar ou atualizar o script RCF, certifique-se de que os seguintes itens estejam disponíveis no switch:

* Cumulus Linux está instalado. Consulte a link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] para versões suportadas.
* Endereço IP, máscara de sub-rede e gateway padrão definidos via DHCP ou configurados manualmente.



NOTE: Você deve especificar um usuário no RCF (além do usuário administrador) para ser usado especificamente para a coleta de logs.
...
| Conjunto | Em portas configuradas para breakout 4x10GbE, uma porta é configurada para breakout 4x25GbE e as demais portas são configuradas para 40/100GbE. Suporta tráfego compartilhado de cluster/HA em portas para nós que utilizam portas compartilhadas de cluster/HA. Consulte a tabela de plataformas no artigo da Knowledge Base link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["Quais plataformas AFF, ASA e FAS usam portas Ethernet de cluster e HA compartilhadas?"^]. Todas as portas também podem ser usadas como portas dedicadas de cluster. 
...
|===
.Versões atuais do script RCF
Existem dois scripts RCF disponíveis para aplicações de Cluster e Armazenamento.  Baixe os RCFs de https://mysupport.netapp.com/site/info/nvidia-cluster-switch["Download do software NVIDIA SN2100"^] página.  O procedimento é o mesmo para todos.

* Cluster: *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*
* Armazenamento: *MSN2100-RCF-v1._x_-Armazenamento*


.Sobre os exemplos
O procedimento de exemplo a seguir mostra como baixar e aplicar o script RCF para switches de cluster.

O comando de exemplo utiliza o endereço IP de gerenciamento do switch 10.233.204.71, máscara de rede 255.255.254.0 e gateway padrão 10.233.204.1.
...
.Observe o seguinte:
====
** Se você estiver atualizando seu RCF, deverá desativar a reversão automática nesta etapa.
** Se você acabou de atualizar sua versão do Cumulus Linux, não precisa desativar a reversão automática para esta etapa, pois ela já está desativada.


====
. Exibir as interfaces disponíveis no switch SN2100:
+
[listing, subs="+quotes"]
----
...
NOTE: Enquanto `scp` Se o método utilizado for o do exemplo, você pode usar o seu método preferido de transferência de arquivos, por exemplo, SFTP, HTTPS ou FTP.
...
----
+
O script RCF executa as etapas listadas no exemplo acima.

+

NOTE: Na etapa 3 *Atualizando o arquivo MOTD* acima, o comando `cat /etc/motd` é executado.  Isso permite verificar o nome do arquivo RCF, a versão do RCF, as portas a serem usadas e outras informações importantes no banner do RCF.

+

NOTE: Para quaisquer problemas com o script Python do RCF que não possam ser corrigidos, entre em contato com https://mysupport.netapp.com/["Suporte NetApp"^] para obter assistência.

. Reaplique quaisquer personalizações anteriores à configuração do switch.  Consultelink:cabling-considerations-sn2100-cluster.html["Analise as considerações sobre cabeamento e configuração."] Para obter detalhes sobre quaisquer alterações adicionais necessárias.
...
5                 5  DWRR 43%
----
. Verifique as informações do transceptor na interface:
+
[listing, subs="+quotes"]
----
...
e3b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. Verifique o estado de saúde do switch no cluster (o switch sw2 pode não ser exibido, pois as LIFs não estão configuradas em e0d).
+
[listing, subs="+quotes"]
----
...
. Repita os passos de 1 a 14 no segundo interruptor.
...
NOTE: Na etapa 3 *Atualizando o arquivo MOTD* acima, o comando `cat /etc/issue.net` é executado.  Isso permite verificar o nome do arquivo RCF, a versão do RCF, as portas a serem usadas e outras informações importantes no banner do RCF.
...
NOTE: Embora `scp` Se o método utilizado for o do exemplo, você pode usar o seu método preferido de transferência de arquivos, por exemplo, SFTP, HTTPS ou FTP.
...
====
.O que vem a seguir?
Após instalar o RCF, você pode link:setup-install-cshm-file.html["Instale o arquivo CSHM"].
