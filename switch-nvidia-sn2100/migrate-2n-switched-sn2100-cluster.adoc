...
summary: Migre um cluster sem switch de dois nós existente para um ambiente de cluster com switches usando os switches de cluster NVIDIA SN2100. 
---
...
[role="lead"]
Migre um cluster sem switch de dois nós existente para um ambiente de cluster com switches usando NVIDIA SN2100 cluster switches. Essa migração não disruptiva permite que você expanda o cluster para além de dois nós.
...
IMPORTANT: Consulte link:cabling-considerations-sn2100-cluster.html["Analise as considerações sobre cabeamento e configuração."] para obter ressalvas e mais detalhes. O link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Switches"^] também contém mais informações sobre cabeamento.
...
* Os nomes dos interruptores SN2100 são _sw1_ e _sw2_.
...
.Cumulus Linux 4.4.x
--
. Desative todas as portas voltadas para os nós (exceto as portas ISL) nos dois novos switches do cluster, sw1 e sw2.
+
...
----
. Verifique se o ISL e as portas físicas no ISL entre os dois switches SN2100, sw1 e sw2, estão ativas nas portas swp15 e swp16:
+
`net show interface`
...
--
====
. [[passo 3]] Verifique se todas as portas do cluster estão ativas:
+
`network port show`
...
----
====
. Desconecte o cabo da porta do cluster e3a no nó 3 e, em seguida, conecte e3a à porta 1 no switch do cluster sw1, usando o cabeamento apropriado suportado pelos switches SN2100.
+
O https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Switches"^] Contém mais informações sobre cabeamento.

. Desconecte o cabo da porta do cluster e3a no nó 4 e, em seguida, conecte e3a à porta 2 no switch do cluster sw1, usando o cabeamento apropriado suportado pelos switches SN2100.
...
.Cumulus Linux 4.4.x
--
. [[passo 8]] No switch sw1, habilite todas as portas voltadas para o nó.
+
...
. [[passo]] No switch sw1, verifique se todas as portas estão ativas:
+
...
. [[passo 9]] No switch sw1, verifique se todas as portas estão ativas:
+
`nv show interface`
...
--
====
. [[passo 10]] Verifique se todas as portas do cluster estão ativas:
+
...
----
====
. Desconecte o cabo da porta do cluster e3b no nó 3 e, em seguida, conecte e3b à porta 1 no switch do cluster sw2, usando o cabeamento apropriado suportado pelos switches SN2100.
. Desconecte o cabo da porta do cluster e3b no nó 4 e, em seguida, conecte e3b à porta 2 no switch do cluster sw2, usando o cabeamento apropriado suportado pelos switches SN2100.
...
.Cumulus Linux 4.4.x
--
. [[passo14]] No switch sw2, habilite todas as portas voltadas para o nó.
+
...
cumulus@sw2:~$ *net commit*
----
. [[passo 15]] No switch sw2, verifique se todas as portas estão ativas:
+
`net show interface all`
...
. [[passo16]] Em ambos os switches sw1 e sw2, verifique se cada um dos nós possui uma conexão com cada switch:
+
`net show lldp`
...
--
====
. [[step17]] Exibir informações sobre os dispositivos de rede descobertos em seu cluster:
+
...
. Ativar reversão automática em todas as LIFs do cluster:
+
...
--
====
. [[passo 8]]Altere o nível de privilégio de volta para administrador:
+
`set -privilege admin`
