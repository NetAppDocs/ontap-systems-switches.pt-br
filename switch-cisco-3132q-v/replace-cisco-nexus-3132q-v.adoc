...
summary: 'A substituição de um switch Cisco Nexus 3132Q-V defeituoso em uma rede de cluster é um procedimento não disruptivo (NDU), e você deve executar uma sequência específica de tarefas.' 
---
...
[role="lead"]
Siga este procedimento para substituir um switch Cisco Nexus 3132Q-V defeituoso em uma rede de cluster. O procedimento de substituição é não disruptivo (NDO).
...
Consulte o link:switch-requirements.html["Requisitos para substituição dos switches de cluster Cisco Nexus 3132Q-V"].
...
** Todas as interfaces lógicas do cluster (LIFs) estão no estado  `up` e foram migradas.
...
** A imagem desejada do RCF e do sistema operacional NX-OS é carregada no switch.
...
Este procedimento substitui o segundo switch de interconexão de cluster Nexus 3132Q-V CL2 pelo novo switch 3132Q-V C2.
...
* n1_clus2 é a primeira interface lógica de cluster conectada ao switch de cluster CL2 ou C2, para o nó n1.
...
+
** Em cada nó, as LIFs de cluster conectadas ao CL2 são migradas para as portas de cluster conectadas ao CL1.
...
----
====
.. Exiba as informações sobre os switches de cluster descobertos:
+
...
. Verifique se o RCF e a imagem apropriados estão instalados no novo switch Nexus 3132Q-V, conforme necessário para suas necessidades, e faça quaisquer personalizações essenciais no site.
+
Você deve preparar o switch de substituição neste momento. Se precisar atualizar o RCF e a imagem, você deve seguir estas etapas:

+
.. No site de suporte da NetApp, consulte link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch/["Switches Ethernet da Cisco"^].
...
--
====
. [[passo 9]]Desligue as portas 1/31 e 1/32 em CL1 e no switch Nexus 3132Q-V ativo:
+
`shutdown`
...
+
As portas Eth1/31 e Eth1/32 devem indicar `(P)`, o que significa que as portas ISL estão ativas no port-channel.
...
----
====
. Em todos os nós, ative todas as portas de interconexão de cluster conectadas ao switch Nexus 3132Q-V C2:
+
`network port modify`
...
[%collapsible]
====
Este exemplo mostra que todas as LIFs foram revertidas com sucesso porque as portas listadas na  `Current Port` coluna têm o status  `true` na coluna  `Is Home`. Se o valor da coluna  `Is Home` for  `false`, a LIF não foi revertida.
