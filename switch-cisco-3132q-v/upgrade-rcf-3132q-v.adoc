...
sidebar: sidebar 
keywords: rcf, upgrade, cluster switch, cisco nexus 3132q-v 
summary: 'Atualize o arquivo de configuração de referência (RCF) nos switches de cluster Cisco Nexus 3132Q-V quando uma versão mais recente estiver disponível.' 
---
= Atualize o RCF nos switches de cluster Cisco Nexus 3132Q-V
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Atualize o arquivo de configuração de referência (RCF) nos switches de cluster Cisco Nexus 3132Q-V quando uma versão mais recente do RCF estiver disponível.
...
NOTE: Ao atualizar ou aplicar um novo RCF, você deve apagar as configurações do switch e realizar a configuração básica.
...
NOTE: Para obter instruções sobre como ativar suas portas 10GbE após uma atualização do RCF, consulte o artigo da Knowledge Base link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["As portas 10GbE em um switch de cluster Cisco 3132Q não ficam online"^].



. Após verificar se as versões do RCF e as configurações do switch estão corretas, copie o  `running-config`arquivo para o arquivo  `startup-config`.
+
Para obter mais informações sobre os comandos da Cisco, consulte o guia apropriado na link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Referência de comandos do Cisco Nexus 3000 Series NX-OS"^]. .Mostrar exemplo
...
----
====
. Reinicie o switch cs2. Você pode ignorar os eventos "portas do cluster inativas" relatados nos nós durante a reinicialização do switch e a saída de erro `% Invalid command at '^' marker`.
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. Reaplique quaisquer personalizações anteriores à configuração do switch. Consulte link:cabling-considerations-3132q-v.html["Analise as considerações sobre cabeamento e configuração."] para obter detalhes sobre quaisquer outras alterações necessárias.
...
----
====
. Repita as etapas 1 a 19 no switch cs1.
...
----
====
. Verifique se as LIFs do cluster retornaram às suas portas originais:
+
`network interface show -vserver Cluster`
...
====
.O que vem a seguir?
Depois de atualizar seu RCF, você link:configure-ssh-keys.html["verificar a configuração SSH"].
