...
summary: Migre um cluster sem switch de dois nós existente para um ambiente de cluster com switches usando os switches de armazenamento Cisco Nexus 9336C-FX2 e 9336C-FX2-T. 
---
= Migre para um cluster comutado de dois nós usando os switches de armazenamento Cisco Nexus 9336C-FX2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Migre um cluster sem switch de dois nós existente para um ambiente de cluster com switches usando os switches de armazenamento Cisco Nexus 9336C-FX2 e 9336C-FX2-T. Essa migração sem interrupções permite que você expanda o cluster para além de dois nós.
...
* Ver link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Switches"^] Para obter mais informações sobre cabeamento.
* Ver link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["Que informações adicionais preciso para instalar meu equipamento que não está no HWU?"^] Para obter mais informações sobre os requisitos de instalação do switch.
* Os cabos Inter-Switch Link (ISL) são conectados às portas 1/35 e 1/36 em ambos os switches 9336C-FX2.
* A personalização inicial dos dois switches 9336C-FX2 foi concluída, de modo que:
+
...
----
====
. Verifique se o ISL e as portas físicas no ISL entre os dois switches 9336C-FX2 cs1 e cs2 estão ativos nas portas 1/35 e 1/36:
+
...
----
====
. Desconecte o cabo da porta do cluster e0a no nó 1 e, em seguida, conecte e0a à porta 1 no switch do cluster cs1, usando o cabeamento apropriado suportado pelos switches 9336C-FX2.
+
O https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - Switches"^] Contém mais informações sobre cabeamento.

. Desconecte o cabo da porta do cluster e0a no nó 2 e, em seguida, conecte e0a à porta 2 no switch do cluster cs1, usando o cabeamento apropriado suportado pelos switches 9336C-FX2.
...
----
====
. Desconecte o cabo da porta do cluster e0b no nó 1 e, em seguida, conecte e0b à porta 1 no switch do cluster cs2, usando o cabeamento apropriado suportado pelos switches 9336C-FX2.
. Desconecte o cabo da porta do cluster e0b no nó 2 e, em seguida, conecte e0b à porta 2 no switch do cluster cs2, usando o cabeamento apropriado suportado pelos switches 9336C-FX2.
...
. Para o ONTAP 9.8 e versões posteriores, habilite o recurso de coleta de logs do monitor de integridade do switch Ethernet para coletar arquivos de log relacionados ao switch, usando os seguintes comandos:
+
...
. Para as versões 9.5P16, 9.6P12 e 9.7P10 do ONTAP e versões de patch posteriores, habilite o recurso de coleta de logs do monitor de integridade do switch Ethernet para coletar arquivos de log relacionados ao switch, usando os seguintes comandos:
+
