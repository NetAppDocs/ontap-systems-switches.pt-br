...
summary: 'Instale o arquivo de configuração de referência (RCF) nos switches de armazenamento Cisco Nexus 9336C-FX2 e 9336C-FX2-T após a configuração inicial para configurar as atribuições de porta e as configurações de rede.' 
---
= Instale o RCF nos switches de armazenamento Cisco Nexus 9336C-FX2 e 9336C-FX2-T
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale o arquivo de configuração de referência (RCF) nos switches de armazenamento Cisco Nexus 9336C-FX2 e 9336C-FX2-T após a configuração inicial. O RCF define as atribuições de porta e as configurações de rede necessárias para a operação da rede de storage.

Consulte o artigo da Knowledge Base link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["Como limpar a configuração de um switch de interconexão Cisco mantendo a conectividade remota"^] para obter mais informações ao instalar seu RCF.
...
* Os switches s1 e s2 estão ligados e a configuração inicial dos switches está concluída (o endereço IP de gerenciamento e o SSH estão configurados).
* A versão desejada do NX-OS foi instalada.
* As portas de armazenamento do nó ONTAP e as portas do compartimento de armazenamento não estão conectadas.
...
[%collapsible]
====
Este exemplo mostra o RCF `NX9336C-FX2-RCF-v1.13-1-Storage.txt` sendo instalado no switch s2:
...
----
====
. Examine a saída do banner do `show banner motd` comando.  Você deve ler e seguir estas instruções para garantir a configuração e o funcionamento corretos do switch.
+
.Mostrar exemplo
[%collapsible]
====
...
. Repita as etapas de 1 a 8 no switch s1.
. Conecte as portas de armazenamento dos nós e as portas dos gabinetes de armazenamento de todos os nós no cluster ONTAP aos switches s1 e s2.
...
.O que vem a seguir?
Após instalar o seu RCF, você podelink:configure-ssh-keys.html["verificar a configuração SSH"]
