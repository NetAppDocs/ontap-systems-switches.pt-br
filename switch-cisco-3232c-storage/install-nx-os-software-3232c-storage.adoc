...
.Antes de começar
Verifique se você possui o seguinte: * Um backup atual da configuração do switch. * Um cluster totalmente funcional (sem erros nos logs ou problemas similares). * link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Página do switch Ethernet da Cisco"^]. Consulte a tabela de compatibilidade do switch para as versões suportadas do ONTAP e NX-OS. * link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Switches Cisco Nexus série 3000"^]. Consulte os guias de software e atualização apropriados disponíveis no site da Cisco para obter a documentação completa sobre os procedimentos de atualização e downgrade do switch Cisco.
