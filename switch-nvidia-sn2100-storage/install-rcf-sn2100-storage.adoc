...
summary: 'Instale ou atualize o script do arquivo de configuração de referência (RCF) nos switches de storage NVIDIA SN2100.' 
---
= Instale ou atualize o script RCF nos switches de storage NVIDIA SN2100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale ou atualize o script do arquivo de configuração de referência (RCF) nos switches de storage NVIDIA SN2100 para configurar as atribuições de porta e as configurações de rede.

.Antes de começar
Antes de instalar ou atualizar o script RCF, certifique-se de que os seguintes itens estejam disponíveis no switch:

* O Cumulus Linux 4.4.3 está instalado.
...
* Agrupamento: *MSN2100-RCF-v1._x_-Cluster*
...
NOTE: O comando de exemplo utiliza o endereço IP de gerenciamento do switch 10.233.204.71, máscara de rede 255.255.254.0 e gateway padrão 10.233.204.1.
...
. Reaplique quaisquer personalizações anteriores à configuração do switch.  Consultelink:cabling-considerations-sn2100-storage.html["Analise as considerações sobre cabeamento e configuração."] Para obter detalhes sobre quaisquer alterações adicionais necessárias.
...
.O que vem a seguir?
Após instalar ou atualizar o RCF, você podelink:setup-install-cshm-file.html["Instale o arquivo CSHM"] .
