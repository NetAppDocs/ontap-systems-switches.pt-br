...
Para a configuração do switch de cluster NVIDIA SN2100, certifique-se de que:
...
NOTE: Consulte link:cabling-considerations-sn2100-storage.html["Considerações sobre cabeamento e configuração"] para obter ressalvas e mais detalhes.
...
----
O exemplo a seguir mostra que as portas ISL estão ativas no switch sw2:
...
----
====
. Em ambos os switches, sw1 e sw2, verifique se cada nó possui uma conexão com cada switch:
+
`net show lldp`
...
--
====
. [[passo 8]] Habilite o recurso de coleta de logs do monitor de integridade do switch Ethernet para coletar arquivos de log relacionados ao switch, usando os comandos:
+
...
. Inicie a funcionalidade de coleta de logs do switch:
+
`system switch ethernet log collect -device *`

+
Aguarde 10 minutos e verifique se a coleta de logs foi bem-sucedida usando o comando:
