...
summary: Se você possui sistemas FAS22xx em um cluster sem switch existente de dois nós, no qual cada módulo controlador tem uma única conexão 10 GbE direta para conectividade do cluster, você pode usar a opção de rede de cluster sem switch e substituir a conectividade direta por conexões de switch. 
---
...
[role="lead"]
Se você possui sistemas FAS22xx em um cluster sem switch existente de dois nós, no qual cada módulo controlador tem uma única conexão 10 GbE direta para conectividade do cluster, você pode usar a opção de rede de cluster sem switch e substituir a conectividade direta por conexões de switch.
...
Este procedimento não é disruptivo e remove a conectividade direta do cluster em um ambiente sem switch, substituindo cada conexão com o switch por uma conexão com o nó parceiro.
...
. Verifique o status de cluster dos nós no console do sistema de qualquer um dos nós:
+
`cluster show`
...
----
====
. Verifique o status do par de HA no console do sistema de qualquer um dos nós: `storage failover show`
+
.Mostrar exemplo
[%collapsible]
====
O exemplo a seguir mostra o status do node1 e do node2:
...
NOTE: A mensagem notifica o suporte técnico sobre esta tarefa de manutenção, de forma que a criação automática de chamados seja suprimida durante a janela de manutenção.
...
----
====
. Verifique se o estado atual do cluster sem switch é  `true`, e então desative o modo de cluster sem switch:
+
...
+
Não importa qual nó seja o nó alvo. Quando o controle é assumido, o nó alvo reinicia automaticamente e exibe a  `Waiting for giveback...` mensagem.

+
O nó ativo agora está fornecendo dados para o nó parceiro (alvo) que foi assumido.

. Aguarde dois minutos após a tomada de controle do nó afetado para confirmar que a tomada de controle foi concluída com sucesso.
. Com o nó de destino exibindo a  `Waiting for giveback...` mensagem, desligue-o.
+
O método que você usa para desligar o nó depende de você usar o gerenciamento remoto por meio do Service Processor (SP) do nó.
...
Está configurado
 a| 
Faça login no nó SP com falha e, em seguida, desligue o sistema: `system power off`
...
Não está configurado
 a| 
No prompt do nó com problemas, pressione  `Ctrl-C` e, em seguida, responda  `y` para interromper o nó.
...
. Conecte a porta de cluster 10 GbE ao switch em ambos os módulos de controlador.
...
. Devolva o armazenamento ao nó de destino:
+
`storage failover giveback -ofnode node2`

. Acompanhe o progresso da operação de giveback:
+
`storage failover show-giveback`

. Após a conclusão da operação de giveback, confirme que o par de HA está saudável e que o takeover é possível:
+
...
[%collapsible]
====
O exemplo a seguir mostra que as LIFs estão `up` no nó1 e no nó2 e que os resultados da coluna "Is Home" são `true`:
